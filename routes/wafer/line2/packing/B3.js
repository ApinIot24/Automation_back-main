...
function getMonthWeek(year, month, week) {
  // Set date ke tanggal 4 (agar pasti dalam bulan tsb)
  let d = new Date(year, month - 1, 4);
  // Jika Minggu => 0 => jadikan 7
  let day = d.getDay() || 7;
...
function getWeekDates(year, month, week) {
  ...
  let arr = [];
  for (let i = 0; i < 7; i++) {
    arr.push(d.toLocaleDateString()); // Hasil default: "MM/DD/YYYY"
...
app.get('/shift1_b3', async (req, res) => {
  try {
    ...
    console.error('Error in GET /shift1_b3:', error.message);
...
app.get('/shift2_b3', async (req, res) => {
  try {
    ...
    console.error('Error in GET /shift2_b3:', error.message);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});
...
app.get('/shift3_b3', async (req, res) => {
  try {
    ...
    // Data jam 0.30 - 6.59 di hari berikutnya
    ...
    console.error('Error in GET /shift3_b3:', error.message);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});

// 4. packing_b3 (data terakhir)
app.get('/packing_b3', async (req, res) => {
  try {
    ...
    console.error('Error in GET /packing_b3:', error.message);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});

// 5. packing_b3_all (graph = 'Y')
app.get('/packing_b3_all', async (req, res) => {
  try {
    ...
    console.error('Error in GET /packing_b3_all:', error.message);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});

// 6. packing_b3_counter (7 data terakhir)
app.get('/packing_b3_counter', async (req, res) => {
  try {
    ...
    console.error('Error in GET /packing_b3_counter:', error.message);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});
...
app.get('/packing_b3_hourly', async (req, res) => {
  try {
    ...
    console.error('Error in GET /packing_b3_hourly:', error.message);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});

// 8. packing_b3_hourly dengan parameter date
app.get('/packing_b3_hourly/date/:date', async (req, res) => {
  try {
    ...
    console.error('Error in GET /packing_b3_hourly/date/:date:', error.message);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});
...
app.get('/packing_b3_daily', async (req, res) => {
  try {
    ...
    console.error('Error in GET /packing_b3_daily:', error.message);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});

// 10. packing_b3_daily dengan parameter date
app.get('/packing_b3_daily/date/:date', async (req, res) => {
  try {
    ...
    console.error('Error in GET /packing_b3_daily/date/:date:', error.message);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});

// 11. packing_b3_weekly (tanpa parameter)
app.get('/packing_b3_weekly', async (req, res) => {
  try {
    let dateNow = new Date();
    let year = dateNow.getFullYear();
    let month = String(dateNow.getMonth() + 1).padStart(2, '0');
    let weeklofmonth = getWeek(dateNow);
    ...
    console.error('Error in GET /packing_b3_weekly:', error.message);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});

// 12. packing_b3_weekly dengan parameter date (misal minggu ke-n)
app.get('/packing_b3_weekly/date/:date', async (req, res) => {
  try {
    ...
    console.error('Error in GET /packing_b3_weekly/date/:date:', error.message);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});
