...
app.get('/packing_a1', async (req, res) => {
    const result = await req.db_iot.query('SELECT * FROM purwosari.packing_a1 ORDER BY id DESC LIMIT 1');
    // console.log("DATA" ,result)
    var datalast = result.rows;
    res.send(datalast);
});
app.get('/packing_a1_all', async (req, res) => {
    const result = await req.db_iot.query(`SELECT * FROM purwosari.packing_a1 where graph = 'Y' ORDER BY id DESC`);
    // console.log("DATA", result)
    var datalast = result.rows;
    res.send(datalast);
});
app.get('/packing_a1_counter', async (req, res) => {
    const result = await req.db_iot.query('SELECT counter FROM purwosari.packing_a1 ORDER BY id DESC LIMIT 7');
    // console.log("DATA", result)
    var datalast = result.rows;
    let arr = [];
...
app.get('/shift3_a1', async (req, res) => {
    ...
    const resulttwo = await req.db_iot.query(`SELECT counter , jam FROM purwosari.packing_a1 where graph = 'Y' AND tanggal = '${thisyestertime}' 
...
app.get('/packing_a1_hourly', async (req, res) => {
    var thisdaytime = format(new Date());
    const result = await req.db_iot.query(`SELECT id , counter,time, jam FROM purwosari.packing_a1 where graph = 'Y' AND tanggal = '${thisdaytime}' 
...
app.get('/packing_a1_hourly/date/:date', async (req, res) => {
    ...
    const result = await req.db_iot.query(`SELECT id ,counter,time, jam FROM purwosari.packing_a1 where graph = 'Y' AND tanggal = '${datethis}' 
    AND jam in ('8.0','9.0','10.0','11.0','12.0','13.0','14.0','14.58','16.0','17.0','18.0','19.0','20.0','21.0','22.0','22.58','23.58') ORDER BY id ASC`);
    var datalast = result.rows;
    var thisdate = new Date(datethis);
    thisdate.setDate(thisdate.getDate() + 1)
    var thisyestertime = format(thisdate)
    const resulttwo = await req.db_iot.query(`SELECT id ,counter,time, jam FROM purwosari.packing_a1 where graph = 'Y' AND tanggal = '${thisyestertime}' 
...
app.get('/packing_a1_daily', async (req, res) => {
    var thisdaytime = format(new Date());
    const result = await req.db_iot.query(`SELECT id ,counter , jam FROM purwosari.packing_a1 where graph = 'Y' AND tanggal = '${thisdaytime}' 
...
app.get('/packing_a1_daily/date/:date', async (req, res) => {
    var datethis = req.params.date
    // console.log(datethis)
    const result = await req.db_iot.query(`SELECT id ,counter, jam FROM purwosari.packing_a1 where graph = 'Y' AND tanggal = '${datethis}' 
    AND jam in ('14.58','22.58') ORDER BY id ASC`);
    //console.log("DATA" ,result)
    var datalast = result.rows;
    var thisdate = new Date(datethis);
    thisdate.setDate(thisdate.getDate() + 1)
    var thisyestertime = format(thisdate)
    const resulttwo = await req.db_iot.query(`SELECT id ,counter, jam FROM purwosari.packing_a1 where graph = 'Y' AND tanggal = '${thisyestertime}' 
...
app.get('/packing_a1_weekly', async (req, res) => {
    ...
    // console.log(datethis)
    const result = await req.db_iot.query(`SELECT id ,counter , jam , realdatetime  FROM purwosari.packing_a1 where graph = 'Y' AND tanggal BETWEEN '${startdate}' AND '${enddate}'
