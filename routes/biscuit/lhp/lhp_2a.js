  ...
  try {
    ...
    if (existingEntry.rows.length > 0) {
      return res.status(400).json({
        message: `Data already exists for this date and shift. your date ${dateformat} and your ${shift}`,
      });
    }
    ...
    planning, bubuk, berat_basah, berat_kering, output_kg, batch_buat,
    variance_batch, variance_fg, batch_tuang, batch_cetak,
...
app.get("/lhpl5_daily/date/:date/:line", async (req, res) => {
  ...
  try {
    ...
    if (isNaN(formattedDate.getTime())) {
      return res
        .status(400)
        .json({ error: "Invalid date. Ensure it follows YYYY-MM-DD format." });
    }
    ...
    if (typeof line !== "string" || line.trim() === "") {
      return res.status(400).json({
        error: "Invalid line. Line parameter must be a non-empty string.",
      });
    }
