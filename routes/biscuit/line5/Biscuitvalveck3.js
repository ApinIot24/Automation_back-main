...
const validateDateParams = [
  param("valve").trim().notEmpty().withMessage("Valve harus diisi"),
...
// Fungsi untuk mengambil status operasional valve dalam rentang tanggal
app.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const result = await safeQuery(query, [valve, startdate, enddate]);
      console.log(result.rows); // Debugging log

      if (result.rows.length === 0) {
        return res.status(404).json({
          message:
            "Tidak ada data operasional valve dalam rentang tanggal tersebut",
...
// Fungsi untuk menghitung durasi operasional valve dalam rentang tanggal
app.get(
...
// Fungsi untuk menampilkan total durasi aktif dan non-aktif valve
app.get(
