...
const validateDateParams = [
  param("agitator").trim().notEmpty().withMessage("Agitator harus diisi"),
...
app.get(
  "/ck_biscuit/agitator/status/:agitator/:startdate/:enddate",
  validateDateParams,
  checkValidation,
  async (req, res) => {
    ...
    try {
      ...
      const result = await safeQuery(query, [agitator, startdate, enddate]);
      console.log(result.rows); // Debugging log

      if (result.rows.length === 0) {
        return res.status(404).json({
          message:
            "Tidak ada data operasional agitator dalam rentang tanggal tersebut",
