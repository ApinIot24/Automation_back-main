...
router.post("/pm_biscuit/submit_pm_checklist/:grup", async (req, res) => {
  ...
  try {
    ...
    // Query untuk mengambil data dari pm_biscuit berdasarkan ID yang diberikan
    const checkQuery = `
    ...
    if (checkResult.rows.length > 0) {
      ...
      const insertQuery = `
        INSERT INTO automation.checklist_pm_biscuit 
        (pm_biscuit_id, week, year, grup, machine_name, part_kebutuhan_alat, equipment, kode_barang, no, periode, periode_start, qrcode)
        VALUES ${values}
        RETURNING *;
      `;
      ...
      // Jika data tidak ditemukan di pm_biscuit
      return res
        .status(400)
        .json({ error: "Data tidak ditemukan di pm_biscuit" });
    }
  } catch (error) {
...
router.put("/pm_biscuit/checklist/:id", async (req, res) => {
  ...
  try {
    ...
    const query = `
      UPDATE automation.checklist_pm_biscuit
      SET
        pic = $1,
        c_i = $2,
        l = $3,
...
router.delete("/pm_biscuit/checklist/:week/:grup", async (req, res) => {
  const { week, grup } = req.params;
  try {
    // Correct the query by passing an array of parameters
    const query = `
      DELETE FROM automation.checklist_pm_biscuit WHERE week = $1 AND grup = $2
    `;
