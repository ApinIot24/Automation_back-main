...
router.post("/pm_biscuit/submit_pm_checklist/:grup", async (req, res) => {
  ...
  try {
    ...
    if (checkResult.rows.length > 0) {
      ...
      const insertQuery = `
        INSERT INTO automation.checklist_pm_biscuit (pm_biscuit_id, week, year, grup, machine_name, part_kebutuhan_alat, equipment, kode_barang, no, periode, periode_start)
        VALUES ${values}
        RETURNING *;
      `;
