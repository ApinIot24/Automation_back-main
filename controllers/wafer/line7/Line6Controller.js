...
// ==== BASIC PACKING L7 ====
export const GetPackingL7 = async (req, res) => {
  const rows = await automationDB.packing_l7.findMany({
...
export const GetPackingL7All = async (req, res) => {
  const rows = await automationDB.packing_l7.findMany({
    select: { id: true, cntr_bandet: true, cntr_carton: true, jam: true, tanggal: true },
    where: { graph: "Y" },
    orderBy: { id: "desc" }
...
export const GetShift_L7 = async (req, res) => {
  const today = new Date();
  const rows = await automationDB.counter_shift_l7.findMany({
...
// ==== SHIFT ====
export const GetShift1L7 = async (req, res) => {
  const today = new Date();
  const rows = await automationDB.packing_l7.findMany({
...
export const GetShift2L7 = async (req, res) => {
  const today = new Date();
  const rows = await automationDB.packing_l7.findMany({
    select: { cntr_bandet: true, cntr_carton: true, jam: true },
...
export const GetShift3L7 = async (req, res) => {
  const today = new Date();
  const next = moment().add(1, "day").toDate();

  const d23 = await automationDB.packing_l7.findMany({
  ...
  const dNext = await automationDB.packing_l7.findMany({
    select: { cntr_bandet: true, cntr_carton: true, jam: true },
...
// ==== SHIFT HOURLY ====
export const GetShift1L7Hourly = async (req, res) => {
  const today = new Date();
...
export const GetShift2L7Hourly = async (req, res) => {
  const today = new Date();
  const date = format(today);
  const isSaturday = today.getDay() === 6;
...
export const GetShift3L7Hourly = async (req, res) => {
  const today = moment();
...
// ==== HOURLY & BY DATE ====
export const GetPackingL7Hourly = async (req, res) => {
...
export const GetPackingL7HourlyByDate = async (req, res) => {
...
// ==== DAILY & WEEKLY ====
export const GetPackingL7Daily = async (req, res) => {
  const today = format(new Date());
  const sql = `
    SELECT id, cntr_bandet, cntr_carton, jam
    FROM automation.packing_l7
...
export const GetPackingL7DailyByDate = async (req, res) => {
  const datethis = req.params.date;
...
export const GetPackingL7Weekly = async (req, res) => {
  const now = new Date();
...
export const GetPackingL7WeeklyByDate = async (req, res) => {
