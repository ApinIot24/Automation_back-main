...
export const getCenkitL1 = async (req, res) => {
    try {
        const result = await automationDB.cenkit_l1.findMany({
            orderBy: { id: 'desc' },
            take: 1
        })
...
export const getCenkitL1All = async (req, res) => {
    try {
        const result = await automationDB.cenkit_l1.findMany({
            where: { graph: 'Y' },
            orderBy: { id: 'desc' }
        ...
        console.error('Error in GET /cenkit_l1_all', error.message);
        res.status(500).send({ error: 'Internal Server Error' });
    }
}
// ==== SHIFT CENKIT L1 ====
export const getShift1CenkitL1 = async (req, res) => {
    try {
        ...
        console.error('Error in GET /shift1_cenkit_l1', error.message);
...
export const getShift2CenkitL1 = async (req, res) => {
    try {
        ...
        console.error('Error in GET /shift2_cenkit_l1', error.message);
        res.status(500).send({ error: 'Internal Server Error' });
    }
}
export const getShift3CenkitL1 = async (req, res) => {
    try {
        ...
        const d23 = await automationDB.cenkit_l1.findMany({
        ...
        const nextRows = await automationDB.cenkit_l1.findMany({
        ...
        console.error('Error in GET /shift3_cenkit_l1', error.message);
        res.status(500).send({ error: 'Internal Server Error' });
    }
}
// ==== HOURLY & By DATE ====
export const getHourlyCenkitL1 = async (req, res) => {
    try {
        ...
        console.error('Error in GET /cenkit_l1_hourly', error.message);
        res.status(500).send({ error: 'Internal Server Error' });
    }
}
export const getHourlyCenkitL1ByDate = async (req, res) => {
    try {
        ...
        const todayRows = await automationDB.cenkit_l1.findMany({
...
// ==== DAILY & WEEKLY ====
export const GetCenkitL1Daily = async (req, res) => {
  try {
    const today = format(new Date());

    const rows = await automationDB.cenkit_l1.findMany({
    ...
    console.error('Error in GET /cenkit_l1_daily', error.message);
    res.status(500).send({ error: 'Internal Server Error' });
  }
};
...
export const GetCenkitL1Weekly = async (req, res) => {
  const now = new Date();
